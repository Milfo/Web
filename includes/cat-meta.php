...
<?php if(isset($options['display_category_meta'])){ ?>
          ...
          <div class="cat-meta entry-header">
            ...
            <span class="author"><img class="cat-meta-ico" src="<?php echo get_template_directory_uri(); ?>/images/author.png" alt="author ico"> <?php the_author_posts_link(); ?></span>
            <?php } ?>
            <?php if($options['display_cat_categories_meta']=='2'){ ?>
            
            <?php } ?>
            <?php if($options['display_cat_date_meta']=='2'){ ?>
						<span class="published"><img class="cat-meta-ico" src="<?php echo get_template_directory_uri(); ?>/images/date.png" alt="date ico"><?php the_time( get_option('date_format') ); ?></span>
            <?php } ?>
            <?php if($options['display_cat_comments_meta']=='2'){ ?>
						<span class="comment-count"><img class="cat-meta-ico" src="<?php echo get_template_directory_uri(); ?>/images/comments.png" alt="comments ico"><?php comments_popup_link(__('No Comments', 'simple'), __('1 Comment', 'simple'), __('% Comments', 'simple')); ?></span>
            <?php } ?>
